...
void LoRa_SendCmd(uint8_t *cmd, uint8_t *result, uint32_t timeOut, uint8_t isPrintf) {
    char *pos;
    HAL_UART_Transmit( &huart7, cmd,strlen((const char *)cmd), 0xff); //发送AT 指令
    HAL_UART_Receive_IT(&huart7,bRxBufferUart1,1);//启动低功耗串口接收中断
    ...
    while(1)
    {
        ...
        if (pos) {
            ...
            LPUART1_RX_LEN=0;
            memset(LPUART1_RX_BUF, 0, strlen((const char *)LPUART1_RX_BUF));//清除缓存
            HAL_UART_Transmit(&huart7, cmd, strlen((const char *) cmd), 0xff); //error 或者无应答，再次发送
            HAL_UART_Receive_IT(&huart7,bRxBufferUart1,1); // 启动低功耗串口接收中断
            HAL_Delay(timeOut);
        }
    }
